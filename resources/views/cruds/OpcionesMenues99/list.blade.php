<x-default-layout>
    ...
    <script>
        document.getElementById('mySearchInput').addEventListener('keyup', function() {
            window.LaravelDataTables['OpcionesMenues99-table'].search(this.value).draw();
        });
        document.addEventListener('livewire:init', function() {
            Livewire.on('success', function() {
                $('#kt_modal_add_OpcionesMenues99').modal('hide');
                window.LaravelDataTables['OpcionesMenues99-table'].ajax.reload();
            });
        });
        $('#kt_modal_add_OpcionesMenues99').on('hidden.bs.modal', function() {
            Livewire.dispatch('new_OpcionesMenues99');
        });
    </script>
    @endpush
