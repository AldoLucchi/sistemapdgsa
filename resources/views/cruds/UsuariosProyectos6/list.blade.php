<x-default-layout>
    ...
    @section('breadcrumbs')
    {{ Breadcrumbs::render('UsuariosProyectos6.index') }}
    @endsection
    ...
    <script>
        document.getElementById('mySearchInput').addEventListener('keyup', function() {
            window.LaravelDataTables['UsuariosProyectos6-table'].search(this.value).draw();
        });
        document.addEventListener('livewire:init', function() {
            Livewire.on('success', function() {
                $('#kt_modal_add_UsuariosProyectos6').modal('hide');
                window.LaravelDataTables['UsuariosProyectos6-table'].ajax.reload();
            });
        });
        $('#kt_modal_add_UsuariosProyectos6').on('hidden.bs.modal', function() {
            Livewire.dispatch('new_UsuariosProyectos6');
        });
    </script>
    @endpush
