<x-default-layout>
    ...
    @section('breadcrumbs')
    {{ Breadcrumbs::render('Contactos4.index') }}
    @endsection
    ...
    <script>
        document.getElementById('mySearchInput').addEventListener('keyup', function() {
            window.LaravelDataTables['Contactos4-table'].search(this.value).draw();
        });
        document.addEventListener('livewire:init', function() {
            Livewire.on('success', function() {
                $('#kt_modal_add_Contactos4').modal('hide');
                window.LaravelDataTables['Contactos4-table'].ajax.reload();
            });
        });
        $('#kt_modal_add_Contactos4').on('hidden.bs.modal', function() {
            Livewire.dispatch('new_Contactos4');
        });
    </script>
    @endpush
