<x-default-layout>
    ...
    @section('breadcrumbs')
    {{ Breadcrumbs::render('Proyectos14.index') }}
    @endsection
    ...
    <script>
        document.getElementById('mySearchInput').addEventListener('keyup', function() {
            window.LaravelDataTables['Proyectos14-table'].search(this.value).draw();
        });
        document.addEventListener('livewire:init', function() {
            Livewire.on('success', function() {
                $('#kt_modal_add_Proyectos14').modal('hide');
                window.LaravelDataTables['Proyectos14-table'].ajax.reload();
            });
        });
        $('#kt_modal_add_Proyectos14').on('hidden.bs.modal', function() {
            Livewire.dispatch('new_Proyectos14');
        });
    </script>
    @endpush
